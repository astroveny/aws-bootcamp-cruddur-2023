...
- Then we create access patterns scripts to fetch the data based on each pattern, create dir: patterns inside dir: ddb
  ...
  The script will retrieve the conversation "generated by the seed script" based on a specific message group uuid
  ...
  This script will list the conversations generated by the seed script
  ...
  ```
  - Add the following query function
  ```python
...
We have to refactor the services to intigrate 'Cognito user id', add access token to the frontend messages pages/components, this will display the message with the user handle, display name and created time.
  ...
  - Create python script [update_cognito_user_ids](https://github.com/astroveny/aws-bootcamp-cruddur-2023/tree/main/backend-flask/bin/db/update_cognito_user_ids) under bin/db.
  - Add the following code to `bin/db/db-setup`: `python "$setup_path/update_cognito_user_ids"`
